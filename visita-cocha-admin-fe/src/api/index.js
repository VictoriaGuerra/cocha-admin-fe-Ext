...
async function safeBackend(fn, fallback) {
  ...
  try {
    ...
    if (code === 'ERR_NETWORK' || msg.toLowerCase().includes('network error') || msg.toLowerCase().includes('connection refused')) {
      console.warn('[api adapter] Backend no disponible. Usando fallback mock.', msg)
      try {
    ...
    console.warn('[api adapter] Fallo backend:', msg)
    throw e
  }
}
